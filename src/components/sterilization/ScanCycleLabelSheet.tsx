...
import { analyzeSterilizationCycle, type ExtractedCycle } from '@/services/aiExtraction';

interface ScanCycleLabelSheetProps {
  visible: boolean;
  onClose: () => void;
  onExtracted: (draft: ExtractedCycle, photoUri: string) => void;
}

/** §aide IA : photo de l'étiquette/ticket autoclave -> extraction via analyze-sterilization-cycle -> pré-remplit CycleFormSheet, jamais d'enregistrement automatique. */
export function ScanCycleLabelSheet({ visible, onClose, onExtracted }: ScanCycleLabelSheetProps) {
  ...
  async function processAsset(uri: string, mimeType: string) {
    ...
    try {
      ...
      const extracted = await analyzeSterilizationCycle(base64, mimeType);
  ...
  return (
    <Sheet visible={visible} onClose={handleClose} title="Scanner une étiquette de cycle">
      <Text style={[styles.intro, { color: theme.textSecondary }]}>
        Photographie l&apos;étiquette ou le ticket imprimé par l&apos;autoclave. Les champs seront pré-remplis — à vérifier avant d&apos;enregistrer, en particulier le résultat.
      </Text>
